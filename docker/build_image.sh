...
Sorgente:
-v <VERSIONE>  : Imposta la versione della release binaria da utilizzare per il build (default: ${LATEST_GOVPAY_FDR_RELEASE})
...
LATEST_LINK="$(curl -qw '%{redirect_url}\n' https://github.com/link-it/govpay-fdr-batch/releases/latest 2> /dev/null)"
LATEST_GOVPAY_FDR_RELEASE="${LATEST_LINK##*/}"
...
[ -n "${CUSTOM_GOVPAY_HOME}" ] && DOCKERBUILD_OPTS=(${DOCKERBUILD_OPTS[@]} '--build-arg' "govpay_fdr_home=${CUSTOM_GOVPAY_HOME}")
[ -n "${CUSTOM_GOVPAY_LOG}" ] && DOCKERBUILD_OPTS=(${DOCKERBUILD_OPTS[@]} '--build-arg' "govpay_fdr_log=${CUSTOM_GOVPAY_LOG}")
if [ -n "${CUSTOM_RUNTIME}" ]
then
...
if [ -n "${LOCALFILE}" ]
then
  DOCKERFILE="govpay-fdr/Dockerfile.daFile"
  cp -f "${LOCALFILE}" buildcontext/
else
  DOCKERFILE="govpay-fdr/Dockerfile.github"
fi
...
if [ -z "$TAG" ] 
then
    REPO=${REGISTRY_PREFIX}/govpay-fdr-batch
  TAGNAME=${VER:-${LATEST_GOVPAY_FDR_RELEASE}}
  TAG="${REPO}:${TAGNAME}"  
fi
